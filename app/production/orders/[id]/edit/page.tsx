...
const productionOrderSchema = z.object({
  ...
  qtyProduced: z.coerce
    .number()
    .min(0, "Quantity produced must be non-negative"),
  ...
  const onSubmit = async (data: ProductionOrderFormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(
          errorData.details || errorData.error || "Failed to update order",
        );
      }
      ...
      toast({
        ...
        description:
          t("orderUpdatedDesc") ||
          "The production order has been successfully updated.",
      });
  ...
  if (!order && !loading)
    return (
      <div className="text-red-500 text-center p-4">
        {t("orderNotFoundOrError", { error: apiError || "" })}
      </div>
    );
    ...
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6">
      <div className="flex items-center justify-between space-y-2">
        <h2 className="text-3xl font-bold tracking-tight">
          {t("editProductionOrderTitle")} - {order?.orderNumber}
        </h2>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {common("orderNumber")}: {order?.orderNumber} | {common("product")}:{" "}
            {order?.productName} | {common("bom")}:{" "}
            {order?.bomName || common("na")} | {t("qtyOrdered")}:{" "}
            {order?.qtyOrdered}
          </CardDescription>
        </CardHeader>
        <CardContent>
