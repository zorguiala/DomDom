...
import { ProductionOrder, RecordProductionDto } from "../../../types/production";

interface UseRecordProductionReturn {
  recordProduction: (orderId: string, data: RecordProductionDto) => Promise<void>;
  loading: boolean;
  error: Error | null;
...
export const useRecordProduction = (): UseRecordProductionReturn => {
  ...
  const recordProduction = async (orderId: string, data: RecordProductionDto): Promise<void> => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error("Failed to record production"));
