...
export class ProductionStatisticsService {
  ...
  async getProductionStatistics(
    statsDto: ProductionStatisticsDto
    ...
    const timeSeries = await this.getProductionTimeSeries(
      startDate,
      endDate,
      bomId,
      employeeId,
    ...
    const byEmployee = employeeId
      ? undefined
      : await this.getProductionByEmployee(startDate, endDate, bomId);
  ...
  private async getProductionMetrics(
    ...
    for (const order of totalCompletedOrders) {
      if (order.actualStartDate && order.completedDate) {
        ...
        const actualHours = differenceInHours(
      ...
      // Quality metrics
      const qualityCheckedRecords = order.productionRecords.filter(
        (record) => record.qualityChecked
      );
    ...
    const efficiency =
      totalCompletedOrders.length > 0 ? totalEfficiency / totalCompletedOrders.length : null;

    const averageCompletionTime =
      totalCompletedOrders.length > 0 ? totalCompletionTime / totalCompletedOrders.length : null;

    const qualityPassRate =
      totalQualityChecked > 0 ? (totalQualityPassed / totalQualityChecked) * 100 : null;
  ...
  private async getProductionByProduct(
    ...
    // Calculate total
    const totalQuantity = Array.from(bomMap.values()).reduce((sum, item) => sum + item.quantity, 0);
  ...
  private async exportToExcel(
    statistics: ProductionStatisticsResult,
    title: string
  ): Promise<Buffer> {
    ...
    overviewSheet.addRow([
      ...
      statistics.metrics.averageCompletionTime.toFixed(2),
    ]);

    if (statistics.metrics.wastagePercentage !== undefined) {
      overviewSheet.addRow([
        'Wastage Percentage',
        `${statistics.metrics.wastagePercentage.toFixed(2)}%`,
      ]);
    }
    ...
    if (statistics.metrics.onTimePercentage !== undefined) {
      overviewSheet.addRow([
        'On-Time Completion',
        `${statistics.metrics.onTimePercentage.toFixed(2)}%`,
      ]);
    }
    ...
    if (statistics.metrics.qualityPassRate !== undefined) {
      overviewSheet.addRow([
        'Quality Pass Rate',
        `${statistics.metrics.qualityPassRate.toFixed(2)}%`,
      ]);
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(12)
          .text(
            `Average Completion Time: ${statistics.metrics.averageCompletionTime.toFixed(2)} hours`
          );

        if (statistics.metrics.wastagePercentage !== undefined) {
          doc
            .fontSize(12)
            .text(`Wastage Percentage: ${statistics.metrics.wastagePercentage.toFixed(2)}%`);
        }

        if (statistics.metrics.onTimePercentage !== undefined) {
          doc
            .fontSize(12)
            .text(`On-Time Completion: ${statistics.metrics.onTimePercentage.toFixed(2)}%`);
        }

        if (statistics.metrics.qualityPassRate !== undefined) {
          doc
            .fontSize(12)
            .text(`Quality Pass Rate: ${statistics.metrics.qualityPassRate.toFixed(2)}%`);
        }
   ...
   */
  private exportToCSV(statistics: ProductionStatisticsResult, title: string): Buffer {
    let csv = `${title}\n\n`;
