...
export class InventoryWastageController {
  constructor(
    private readonly wastageService: InventoryWastageService
  ) {}

  @Post()
  async create(
    @Body() createWastageDto: CreateWastageRecordDto,
  ...
  async getWastageSummaryByReason(
    ...
    return this.wastageService.getWastageSummaryByReason(
      new Date(startDate),
      new Date(endDate)
    );
  }
  ...
  async getWastageByProduct(
    ...
    return this.wastageService.getWastageByProduct(
      new Date(startDate),
      new Date(endDate)
    );
  }
