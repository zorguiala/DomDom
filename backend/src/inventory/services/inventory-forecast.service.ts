...
export class InventoryForecastService {
  ...
  private async calculateHistoricalUsage(productId: string): Promise<{
    ...
    const earlierUsage = earlierTransactions.reduce(
      (sum, tx) => sum + Number(tx.quantity), 
      0
    );
    ...
    if (recentUsage > earlierUsage * 1.2) { // 20% increase
      trend = 'INCREASING';
    } else if (recentUsage < earlierUsage * 0.8) { // 20% decrease
      trend = 'DECREASING';
    }
  ...
  async getCriticalLowStockProducts(): Promise<InventoryForecast[]> {
    ...
    return allForecasts.filter(forecast => 
      forecast.isLow && forecast.daysRemaining <= 7
    );
  }
    ...
    const suggestions = allForecasts
      ...
      .map(forecast => {
        ...
        if (costPrice > 0) {
          ...
          economicOrderQuantity = Math.ceil(
            Math.sqrt(
              (2 * annualDemand * orderCost) / (holdingCostPercent * costPrice)
            )
          );
        }
      ...
      .sort((a, b) => {
        ...
        if (a.priority !== b.priority) {
          return a.priority === 'HIGH' ? -1 : a.priority === 'MEDIUM' ? (b.priority === 'HIGH' ? 1 : -1) : 1;
        }
  ...
  async getLowStockAlerts() {
    ...
    .sort((a, b) => {
      ...
      if (a.severity !== b.severity) {
        return a.severity === 'CRITICAL' ? -1 : 
          a.severity === 'WARNING' ? (b.severity === 'CRITICAL' ? 1 : -1) : 1;
      }
  ...
  async getOptimizationMetrics() {
    ...
    for (const product of products) {
      ...
      if (forecast) {
        ...
        const risk = forecast.isLow ? 
          (forecast.daysRemaining <= 7 ? 100 : 50) : 
          (forecast.averageDailyUsage > 0 ? 25 : 0);
    ...
    const inventoryTurnoverRate = totalInventoryValue > 0 ? 
      totalAnnualUsageValue / totalInventoryValue : 0;
    ...
    const averageStockoutRisk = products.length > 0 ? 
      totalStockoutRisk / products.length : 0;
  ...
  async getUsageTrend(productId: string, period: number = 90) {
    ...
    for (let i = 1; i <= 30; i++) { // Forecast next 30 days
